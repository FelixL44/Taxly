...
const HouseholdInformation: React.FC<{ onBack: () => void }> = () => {
  const [expanded, setExpanded] = useState(true);
  const [hasSingleHousehold, setHasSingleHousehold] = useState(true);
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" sx={{ color: INACTIVE_COLOR, mb: 4, lineHeight: 1.6 }}>
        Angaben zum Haushalt sind sinnvoll, wenn Sie mit weiteren alleinstehenden Personen zusammenleben. Jede Person ka{' '}
        <Typography
          component="span"
            ...
            <Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                <Typography variant="body2" sx={{ fontWeight: 500, color: '#1E293B' }}>
                  Haben Sie 2025 an mindestens einem Tag einen Einzelhaushalt geführt?
                </Typography>
                <Tooltip
                  title="Ein Einzelhaushalt liegt vor, wenn Sie allein leben und keine weiteren Personen im Haushalt wohnen."
                  arrow
                >
              ...
              <RadioGroup
                row
                value={hasSingleHousehold ? 'yes' : 'no'}
                onChange={(e) => setHasSingleHousehold(e.target.value === 'yes')}
                sx={{
